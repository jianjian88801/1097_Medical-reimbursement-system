                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('caikuai','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/yiliaobaoxiaoxitong/upload/caikuaiMuBan.xls"
                        >批量导入财会审核人员数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('caikuai','导入导出')"
                                ...
                                :on-success="caikuaiUploadSuccess"
                                :on-error="caikuaiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('caikuai','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入财会审核人员数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('caikuai','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "caikuai.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('caikuai','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('caikuai','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('caikuai','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>

                            <el-button v-if="isAuth('caikuai','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'caikuai',//当前表表名,
                        sumColum : 'caikuai_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'caikuai_name',//分组字符串字段
                        // types : 'caikuai_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['caikuaiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            resetPassword(id) {
                //重置密码
                this.$http({
                    url: "caikuai/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            caikuaiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "caikuai/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
                            message: "导入财会审核人员数据成功",
                            type: "success",
                            duration: 1500,
                            onClose: () => {
                                _this.search();
